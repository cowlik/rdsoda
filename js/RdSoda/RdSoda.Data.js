...
RdSoda.Data = function(callback) {
	...
	function init() {
		...
		data.Instagram.requestEndpointData('tags/'+ instagramConfig.tag +'/media/recent/', onInstagramDataReady);
	}
		...
		if (instagramData.data.length > 0) {
			...
			if (nextReqURL != undefined) {
				...
				if (counter <= instagramConfig.MAX_API_REQS) {
					...
					data.Instagram.requestEndpointData(nextReqURL, onInstagramDataReady);
				} else {
					sortResults();
				}
